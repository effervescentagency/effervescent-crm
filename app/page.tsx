...
const SYSTEMS = [
  ...
  },
  {
    ...
    description: "New recruitment CRM - candidates and hiring pipeline.",
  ...
  {
    name: "Basecamp",
    description: "Sign in to Basecamp.",
...
export default async function Home() {
  ...
  return (
    <div className="min-h-screen bg-pink-50">
      <div className="bg-pink-400 px-6 py-6 shadow-sm">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div>
            <div className="text-white font-bold text-xl">Effervescent</div>
            <div className="text-pink-50 text-sm">Staff Dashboard</div>
          </div>
          {session?.user?.email && (
          ...
          {SYSTEMS.map((system) =>
            system.available ? (
              <a
                ...
                className="block bg-white border border-pink-100 rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-pink-300 transition"
              >
                <div className="w-12 h-12 rounded-xl bg-pink-200 border border-pink-300 mb-4" />
